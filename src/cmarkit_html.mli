...
val of_doc : ?backend_blocks:bool -> safe:bool -> Oymarkit.Doc.t -> string
...
(** [renderer ~safe ()] is the default HTML renderer. This renders the
    ...
    {{!Oymarkit.extensions}extensions}.
    ...
    {ul
    {- [safe], if [true] {{!Oymarkit.Block.extension-Html_block}HTML blocks} and
       {{!Oymarkit.Inline.extension-Raw_html}raw HTML inlines} are discarded and
       replaced by an HTML comment in the output. Besides the URLs of
       autolinks, links and images that satisfy
       {!Oymarkit.Inline.Link.is_unsafe} are replaced by the empty string.
...
(** {1:integration HTML integration notes}
    ...
    code block with
    {!Oymarkit.Block.Code_block.language_of_info_string}, a
    ...
    If your document has {!Oymarkit.Inline.extension-Ext_math_span}
    inlines or {!Oymarkit.Block.extension-Ext_math_block} blocks, the
