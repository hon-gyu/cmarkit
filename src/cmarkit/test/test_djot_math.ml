...
(** Djot's math spelling, behind [backtick_math]: a verbatim span prefixed with [$]
...
let html ?backtick_math s =
  let doc = Doc.of_string ~strict:false ?backtick_math s in
  print_string (Cmarkit_html.of_doc ~safe:false doc)

let commonmark ?backtick_math s =
  let doc = Doc.of_string ~strict:false ?backtick_math s in
...
let%expect_test "on: [$`x`] is inline math" =
  html ~backtick_math:true "a $`e=mc^2` b";
...
let%expect_test "on: math round-trips through CommonMark" =
  ...
  commonmark ~backtick_math:true "a $`e=mc^2` b";
  [%expect {| a $e=mc^2$ b |}]
