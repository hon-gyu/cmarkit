...
(** Djot emphasis classification, behind [djot_emphasis].
...
let html ?djot_emphasis ?intraword_emphasis s =
  let doc = Doc.of_string ~strict:false ?djot_emphasis ?intraword_emphasis s in
  print_string (Cmarkit_html.of_doc ~safe:false doc)

let%expect_test "both: the ordinary cases agree" =
  html "*a* and _b_";
  html ~djot_emphasis:true "*a* and _b_";
  [%expect {|
...
let%expect_test "djot: [intraword_emphasis] is what keeps snake_case intact" =
  ...
  html ~djot_emphasis:true "snake_case_here";
  html ~djot_emphasis:true ~intraword_emphasis:false "snake_case_here";
  [%expect {|
...
let%expect_test "djot: [_] and [*] behave the same intraword" =
  html ~djot_emphasis:true "a*b*c";
  html ~djot_emphasis:true "a_b_c";
  [%expect {|
    <p>a<em>b</em>c</p>
    <p>a<em>b</em>c</p>
    |}]
