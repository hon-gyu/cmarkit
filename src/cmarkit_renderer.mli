...
val doc_to_string : t -> Oymarkit.Doc.t -> string
(** [doc_to_string r d] renders document [d] to a string using renderer [r]. *)

val buffer_add_doc : t -> Buffer.t -> Oymarkit.Doc.t -> unit
...
type inline = context -> Oymarkit.Inline.t -> bool
...
type block = context -> Oymarkit.Block.t -> bool
...
type doc = context -> Oymarkit.Doc.t -> bool
...
val init_context : t -> (context -> Oymarkit.Doc.t -> unit)
...
module Context : sig
  ...
  val get_doc : t -> Oymarkit.Doc.t
  (** [get_doc c] is the document being rendered. *)

  val get_defs : t -> Oymarkit.Label.defs
  ...
  val init : t -> Oymarkit.Doc.t -> unit
...
(** {1:example Extending renderers}
    ...
    {- Extend the abstract syntax tree with a [Doc] block which
       ...
       splicing is already built-in via the {!Oymarkit.Block.extension-Blocks}
       block case).}
    {- Change the rendering of {!Oymarkit.Inline.extension-Image} inlines to
...
let media_link c l =
  ...
  match Oymarkit.Inline.Link.reference_definition defs l with
  | Some Oymarkit.Link_definition.Def (ld, _) ->
      let start_tag = match Oymarkit.Link_definition.dest ld with
