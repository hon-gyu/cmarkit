...
val doc_to_string : t -> Omarkit.Doc.t -> string
(** [doc_to_string r d] renders document [d] to a string using renderer [r]. *)

val buffer_add_doc : t -> Buffer.t -> Omarkit.Doc.t -> unit
...
type inline = context -> Omarkit.Inline.t -> bool
...
type block = context -> Omarkit.Block.t -> bool
...
type doc = context -> Omarkit.Doc.t -> bool
...
val make :
  ?init_context:(context -> Omarkit.Doc.t -> unit) ->
...
val init_context : t -> (context -> Omarkit.Doc.t -> unit)
...
module Context : sig
  ...
  val get_doc : t -> Omarkit.Doc.t
  (** [get_doc c] is the document being rendered. *)

  val get_defs : t -> Omarkit.Label.defs
...
(** {1:example Extending renderers}
    ...
    {- Extend the abstract syntax tree with a [Doc] block which
       ...
       splicing is already built-in via the {!Omarkit.Block.extension-Blocks}
       block case).}
    {- Change the rendering of {!Omarkit.Inline.extension-Image} inlines to
...
let media_link c l =
  ...
  match Omarkit.Inline.Link.reference_definition defs l with
  | Some Omarkit.Link_definition.Def (ld, _) ->
      let start_tag = match Omarkit.Link_definition.dest ld with
