(** {0 Block AST Properties}
    - Property types
    - Test execution printer (for counterexamples)
    - QCheck.Test bridge *)
...
(** Check property and pp failure
    @raise Assert_failure if the property passes *)
...
(** String representation of a block for {b comparison}. *)
let canonical (b : Block.t) : string =
   ...
   fun b ->
    let b' =
      reparse ?emphasis_delims ?strong_emphasis_delims ?intraword_emphasis
        ...
        ?smart_punctuation b
    in
    if String.equal (canonical b) (canonical b') then Pass
    else
      Fail (b, [ ("reparse", Block b'); ("reparse_cm", Md (to_commonmark b')) ])
  in
  { name = "roundtrip"; check }

let roundtrip : t = roundtrip_with ()

(** [to_commonmark b = to_commonmark (parse (to_commonmark b))] *)
let render_determinism : t =
  {
...
let uniformity_with ~(name : string) ~(wrap : Block.t -> Block.t)
    ~(unwrap : Block.t -> Block.t option) : t =
  {
    name;
    check =
      (fun b ->
